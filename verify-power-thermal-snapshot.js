...
(async () => {
  ...
  ship.componentHeatRadiated = [7, 11];
  ...
  assert.equal(heatOnly.powerThermal.componentHeatRate, 6);
  close(heatOnly.powerThermal.cooling, 1.5, "Heat-only cooling converts amount once");
  close(heatOnly.powerThermal.netHeatRate, 4.6, "Heat-only net Heat rate");
  ...
  assert.equal(merged.powerThermal.componentHeatRate, 6, "client merge receives newest powerThermal");
  close(merged.powerThermal.netHeatRate, 4.6, "client merge receives refreshed net Heat");
  ...
  assert.equal(retained.powerThermal.componentHeatRate, 6, "omitted powerThermal retains cached value");
