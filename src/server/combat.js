...
function applyBlastDamageToShip(room, target, origin, cfg, damageMultiplier, contactTargetShip, attackerId, now) {
  ...
  const base = isContact
    ? (cfg.directContactHullDamage || cfg.splashCentreDamage * cfg.directContactMultiplier)
    : cfg.splashCentreDamage;
  const hullBudget = base * falloff * damageMultiplier;
  if (hullBudget <= 0) return 0;

  const maxComponents = isContact ? cfg.contactMaxAffectedComponents : cfg.splashMaxAffectedComponents;
  const internalReduction = isContact ? cfg.contactInternalDamageReduction : cfg.splashInternalDamageReduction;
  ...
  if (isContact) {
    ...
    for (const alloc of allocations) {
      ...
      if (!alloc.exposed && !alloc.isArmour) dmg *= (1 - internalReduction);
      if (dmg > 0) {
        const dealt = applyDirectComponentDamage(room, target, alloc.index, dmg, attackerId, now, {});
        totalRemoved += dealt;
      }
    }
  }
  ...
  for (const kind of ["ships", "drones", "projectiles"]) {
    ...
    for (const entity of out) {
      ...
      if (kind === "ships") {
        ...
        const damage = blastDamageFor(edge, blastR, cfg.splashCentreDamage, exp) * damageMultiplier;
