...
export function connect(url, onOpenCallback, options = {}) {
  ...
  state.telemetryFocusLastSentGeneration = null;
  // telemetryFocusLastSentShipId is intentionally preserved so a reconnect can
  // explicitly clear the server's previous focus even when the desired focus is null.
  let socket;
  let attempt = null;
  try {
