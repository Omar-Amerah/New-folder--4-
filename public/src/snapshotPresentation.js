...
// a command change on every frame forever.
function movementTogglesChanged(previous, next) {
  if (!previous || !next) return previous !== next;
  for (const field of MOVEMENT_TOGGLE_FIELDS) {
...
// Station production and the central launch corridor advance while a build is active, so the queue is
...
function productionSignature(station) {
  ...
  const queueSignature = Array.isArray(queue)
    ...
    : "";
  const launchSignature = launches.map((launch) => `${launch.shipId}:${Math.round((Number(launch.progress) || 0) * 100)}:${launch.doorOpen ? 1 : 0}`).join(",");
...
function selectedTelemetryChanges(previousIndex, nextIndex, selectedIds) {
  ...
  for (const id of selectedIds || []) {
    ...
    result.command ||= fieldsChanged(previous, next, COMMAND_FIELDS)
      || movementTogglesChanged(previous?.movementToggles, next?.movementToggles);
  }
  return result;
}
