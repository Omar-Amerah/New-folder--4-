...
function resetInteraction(clearSelection = true) { const view = ui(); view.sourceIndex = null; view.path = []; view.hoverCell = null; if (clearSelection) { view.selectedIndex = null; view.selectedConnectionKey = null; view.selectedSectionId = null; } }
...
function cancelDrawing() { ui().sourceIndex = null; ui().path = []; ui().hoverCell = null; refreshWiringPresentation(); }
...
export function clearWiringPresentation() { dom.wiringOverlayHost?.replaceChildren(); dom.grid?.classList.remove("wiring-overlay-active"); if (dom.wiringStatusPanel) { dom.wiringStatusPanel.hidden = true; dom.wiringStatusPanel.innerHTML = ""; } ui().hoverCell = null; }
function refreshToolbar() {
  ...
  if (dom.wiringHint) dom.wiringHint.textContent = ui().sourceIndex == null ? "Select a source to begin, or select existing wiring to inspect it." : "Click adjacent occupied cells. Click the previous cell or right-click to step back; Escape cancels.";
}
...
function renderWiringOverlay() {
  ...
  const svg = svgEl("svg", { viewBox: `0 0 ${GRID_SIZE} ${GRID_SIZE}` }, "wiring-overlay"); const selected = selectedConnection();
  for (const section of bucket().sections) {
    const users = bucket().connections.filter((connection) => connection.sectionIds.includes(section.id)); const isSelected = selected && users.some((connection) => rules().connectionKey(connection) === rules().connectionKey(selected));
    svg.appendChild(line(section, `wire-${view.mode}${isSelected ? " wire-net-selected" : ""}`));
