...
const documentHandlers=new Map(); const windowHandlers=new Map();
...
state.selectedShipIds=new Set(['selected']); event=keyEvent('o'); input.handleKeyDown(event); assert.deepEqual(sent.pop(),{type:'rotate',direction:1,active:true,shipIds:['selected']},'O starts rotating the selected ship');
state.selectedShipIds=new Set(['unselected']); input.handleKeyUp(keyEvent('o')); assert.deepEqual(sent.pop(),{type:'rotate',direction:1,active:false,shipIds:['selected']},'O release targets the ship latched on keydown');
input.bindArenaPointerListeners(c2); state.selectedShipIds=new Set(['selected']); input.handleKeyDown(keyEvent('i')); assert.deepEqual(sent.pop(),{type:'rotate',direction:-1,active:true,shipIds:['selected']});
windowHandlers.get('blur')(); assert.deepEqual(sent.pop(),{type:'rotate',direction:-1,active:false,shipIds:['selected']},'window blur releases latched rotation');
assert(!state.keys.has('i'),'window blur clears the rotation key state');
input.unbindArenaPointerListeners();
