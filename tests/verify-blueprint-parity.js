...
(async () => {
  ...
  const defaultStats = serverStats.computeStats(DEFAULT_DESIGN, DEFAULT_WIRING);
  ...
  assert(defaultStats.unitCost >= 350 && defaultStats.unitCost <= 460, `default unit cost expected near $400 and below starting money, got ${defaultStats.unitCost}`);
  ...
  assert.strictEqual(defaultStats.cost, 204, "default component cost total matches the defensive stock design");
