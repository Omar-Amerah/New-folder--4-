...
const CANONICAL_DESCRIPTIONS = Object.freeze({
  ...
  thermalInductionLance: "Targeting: Prioritises functioning Power generators when available, then other active systems. Zero-damage induction beam that injects Heat into a target component and nearby components. Sustained contact increases Heat transfer; shields reduce coupling, while Refractory Armour can block the beam.",
...
(async () => {
  ...
  assert.match(repairText, /Linear nominal output|add their nominal rates linearly/);
  assert.doesNotMatch(repairText, /diminishing returns|stack rank|80%/i);
