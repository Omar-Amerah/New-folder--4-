...
const blue2BeforeLeave = stableBeforeLeave.spawns.find((spawn) => spawn.playerId === "blue2");
stable.players.delete("blue3");
const stableAfterLeave = getSpawnRegionPlan(stable);
assert.strictEqual(stableAfterLeave, stableBeforeLeave, "frozen match plan survives roster changes");
assert.deepStrictEqual(stableAfterLeave.spawns.find((spawn) => spawn.playerId === "blue2"), blue2BeforeLeave, "teammate leaving does not move a surviving player's spawn");
invalidateSpawnPlan(stable);
assert.notDeepStrictEqual(getSpawnRegionPlan(stable).spawns.find((spawn) => spawn.playerId === "blue2"), blue2BeforeLeave, "explicit invalidation permits a new-match layout");
