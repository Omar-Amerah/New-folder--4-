...
function makePlayer(id){ return { id, name:id, color:'#39f', team:'blue', isBot:false, connected:true, ready:false, money:0, income:0, earned:0, spent:0, shipCap:5, deployedFleetCost:0, destroyedEnemyCost:0, kills:0, losses:0, captures:0, ships:[], design:[{type:'core'}], stats:{unitCost:1}, shipsBuilt:0, lostFleetCost:0, rallyPoint:{x:0,y:0} }; }
...
function room(){ const pa=makePlayer('pa'), pb=makePlayer('pb'), pc=makePlayer('pc'); const s=makeShip('s','pa'); pa.ships.push(s); return { code:'R', phase:'active', adminId:'pa', stateEpoch:1, snapshotSeq:0, staticRevision:1, componentCatalogueRevision:1, mapSizeLabel:'tiny', world:{width:100,height:100}, map:{seed:1,asteroids:[]}, rules:{gameMode:'solo'}, winner:null, matchStartedAt:1, bullets:[], effects:[], points:[], controlVictory:null, players:new Map([[pa.id,pa],[pb.id,pb],[pc.id,pc]]), ships:new Map([[s.id,s]]), clients:new Set() }; }
