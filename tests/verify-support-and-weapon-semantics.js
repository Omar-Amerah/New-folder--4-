...
{
  const pd = makeShip("pd", "a", 0, 0, [{ x:7,y:7,type:"core" }, { x:7,y:6,type:"reactor" }, { x:7,y:5,type:"interceptorPod", rotation:0 }]); const enemy = makeShip("enemy", "b", 120, 0, localDesign); const r = room([pd, enemy]); r.bullets.push({ id:"missile-1", type:"missile", interceptable:true, ownerId:"b", targetId:pd.id, x:60, y:0, life:5, hp:10 }); updateShipWeapons(r, pd, [pd,enemy], 0.1, 1000); assert.strictEqual(r.bullets.at(-1).targetId, "missile-1", "projectile PD shot targets projectile id"); r.bullets = []; pd.weaponCooldowns[2] = 0; updateShipWeapons(r, pd, [pd,enemy], 0.1, 1000 + BALANCE.fleetDefence.baseReacquisitionDelayMs); updateShipWeapons(r, pd, [pd,enemy], 0.1, 1000 + BALANCE.fleetDefence.baseReacquisitionDelayMs * 2); assert.strictEqual(r.bullets.at(-1).targetId, enemy.id, "ship fallback PD shot targets ship id"); assert.strictEqual(findPointDefenseTarget(r, 0, 0, "a", PARTS.pointDefense.weapon, [pd, enemy], pd.id).entity.id, enemy.id);
}
{
