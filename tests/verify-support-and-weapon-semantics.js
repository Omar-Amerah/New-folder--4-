...
function room(ships) {
  ...
  const r = { phase: "active", rules: { gameMode: "teams" }, players, ships: new Map(ships.map(s => [s.id, s])), nextEntityId: 1, bullets: [], effects: [], map: { asteroids: [], safeZones: [], relays: [] }, points: [], world: { width: 2000, height: 2000 }, combatRandom: () => 0 };
...
{
  const s = weaponShip(); const e = makeShip("enemy", "b", 180, 90, localDesign); const r = room([s, e]); s.componentHeatState[2] = STATE.OVERHEATED; s.weaponCooldowns = [0,0,0]; updateShipWeapons(r, s, [s, e], 0.25, 1000); assert.strictEqual(s.weaponAimTargetIds[2], e.id); assert.notStrictEqual(s.weaponAngles[2], 0, "overheated powered weapon tracks"); assert.strictEqual(r.bullets.length, 0); assert.strictEqual(s.weaponCooldowns[2], 0); s.componentHeatState[2] = STATE.NORMAL; for (let i=0;i<20 && r.bullets.length===0;i++) updateShipWeapons(r, s, [s,e], 0.1, 1400 + i); assert(r.bullets.length > 0, "restored thermal activity permits firing"); s.componentHp[2] = 0; updateShipWeapons(r, s, [s,e], 0.1, 2000); assert.strictEqual(s.weaponAimTargetIds[2], null, "destroyed weapon cannot aim");
}
{
...
{
  const pd = makeShip("pd", "a", 0, 0, [{ x:7,y:7,type:"core" }, { x:7,y:6,type:"reactor" }, { x:7,y:5,type:"interceptorPod", rotation:0 }]); const enemy = makeShip("enemy", "b", 120, 0, localDesign); const r = room([pd, enemy]); r.bullets.push({ id:"missile-1", type:"missile", interceptable:true, ownerId:"b", targetId:pd.id, x:60, y:0, life:5, hp:10 }); updateShipWeapons(r, pd, [pd,enemy], 0.1, 1000); assert.strictEqual(r.bullets.at(-1).targetId, "missile-1", "projectile PD shot targets projectile id"); r.bullets = []; pd.weaponCooldowns[2] = 0; updateShipWeapons(r, pd, [pd,enemy], 0.1, 1600); updateShipWeapons(r, pd, [pd,enemy], 0.1, 2200); assert.strictEqual(r.bullets.at(-1).targetId, enemy.id, "ship fallback PD shot targets ship id"); assert.strictEqual(findPointDefenseTarget(r, 0, 0, "a", PARTS.pointDefense.weapon, [pd, enemy], pd.id).entity.id, enemy.id);
}
{
  const oldEnv = process.env.NODE_ENV; const oldWarn = console.warn; const warnings = []; console.warn = (m) => warnings.push(String(m));
  try { const s = makeShip("hp", "a", 0, 0, localDesign); const r = room([s]); const nonCore = s.componentHp.reduce((sum,h,i)=>s.design[i].type === "core" ? sum : sum + h, 0); s.hp = nonCore - 25; process.env.MFA_ASSERT_COMPONENT_HP = "1"; process.env.NODE_ENV = "development"; tickRoom(r, 0.001, 1000); assert(warnings.some(w => w.includes("hp drift"))); assert.strictEqual(s.hp, nonCore); assert.strictEqual(s.componentHp[0], s.componentMaxHp[0], "core HP remains excluded"); warnings.length = 0; s.hp = nonCore - 25; process.env.MFA_ASSERT_COMPONENT_HP = "0"; process.env.NODE_ENV = "production"; tickRoom(r, 0.001, 1010); assert.strictEqual(s.hp, nonCore - 25); assert.strictEqual(warnings.length, 0); } finally { process.env.NODE_ENV = oldEnv; console.warn = oldWarn; }
}
