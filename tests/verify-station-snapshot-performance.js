...
(async () => {
  ...
  assert.equal(alliedFull.hangars?.length, 3, "full station baseline carries all three hangar records");
  ...
  assert.equal(alliedCompact.hangars, undefined, "compact station update omits cached hangar geometry");
  assert.equal(alliedCompact.hangar, undefined, "compact station update carries no singular hangar field");
  ...
  assert.deepEqual(mergedAllied.hangars, alliedFull.hangars, "client reconstructs all three hangar geometries through compact updates");
