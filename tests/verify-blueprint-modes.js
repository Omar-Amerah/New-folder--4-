...
(async () => {
  ...
  state.selectedPart = "frame"; state.selectedPartCategory = "Structure"; state.previewRotation = 0; designer.setBlueprintView("build"); paletteUi.renderPalette(); const beforeBuild = state.design.length; clickCell(grid, 9, 8); assert.ok(state.design.length > beforeBuild, "Build primary click edits physical design"); const withLeaf = clone(state.design); assert.equal(clickCell(grid, 9, 8, 2), false, "Build removal prevents the browser context menu"); assert.ok(state.design.length < withLeaf.length, "Build right-click removes"); designer.undoBlueprintEdit();
  designer.setBlueprintView("heat"); state.selectedPart = "armor"; state.selectedPartCategory = "Structure"; state.previewRotation = 0; const beforeHeat = JSON.stringify(state.design); clickCell(grid, 9, 8); assert.notEqual(JSON.stringify(state.design), beforeHeat, "Heat primary click edits physical design"); const heatLength = state.design.length; assert.equal(clickCell(grid, 9, 8, 2), false, "Heat removal prevents the browser context menu"); assert.equal(state.design.length, heatLength - 1, "Heat right-click removes"); state.selectedPart = "blaster"; state.selectedPartCategory = "Weapons"; state.previewRotation = 0; const rotBefore = state.previewRotation; designer.rotateFocusedPart(); assert.notEqual(state.previewRotation, rotBefore, "Heat R path rotates preview");
