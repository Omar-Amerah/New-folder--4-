...
function runServerSpatialPerformanceBenchmark() {
  const baseline = measure(false);
  const optimized = measure(true);
  const fixture = { ships: 80, drones: 240, bullets: 900, effects: 0, tickBudgetMs: +TICK_BUDGET_MS.toFixed(3) };
  const reductions = {
  ...
  return { fixture, baseline, optimized, reductions };
}

module.exports = { runServerSpatialPerformanceBenchmark };
