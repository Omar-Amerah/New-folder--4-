...
(async () => {
  ...
  try {
    ...
    await bot.open();
    bot.send({ type: "join", room: ROOM, name: "HeatBot", team: "red", protocolVersion:6, minProtocolVersion:6, maxProtocolVersion:6, capabilities:["messagepack", "entityDeltaSnapshotsV1"] });
    ...
    await page.evaluate(({ design, dataLinks }) => window.__mfaNetSend({ type: "deploy", design, dataLinks, combatStyle: "sentry" }), { design: DESIGN, dataLinks: [] });
    bot.send({ type: "deploy", design: DESIGN, dataLinks: [], combatStyle: "sentry" });
    await until(() => bot.latest.state?.phase === "active", "active phase");
    diagnostics.phases.push({ phase: "active", at: Date.now() });
    await page.evaluate(({ design, dataLinks }) => window.__mfaNetSend({ type: "buyShip", design, dataLinks, count: 1, requestId: `h-${Date.now()}-b`, combatStyle: "sentry" }), { design: DESIGN, dataLinks: [] });
    bot.send({ type: "buyShip", design: DESIGN, dataLinks: [], count: 1, requestId: `h-${Date.now()}-t`, combatStyle: "sentry" });
