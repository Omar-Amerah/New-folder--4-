...
(async () => {
  const [{ state }, storage, designer, history, wiringUi] = await Promise.all([
    import("./public/src/state.js"), import("./public/src/design/blueprintStorage.js"), import("./public/src/ui/designerUi.js"), import("./public/src/design/blueprintEditHistory.js"), import("./public/src/ui/wiringUi.js")
  ]);
  let persistCalls = 0;
  ...
  designer.setBlueprintView("build"); assert.equal(elements.get("buildInteractionGuide").hidden, false, "Build guide visible"); assert.match(elements.get("buildInteractionGuide").textContent, /right-click/i);
  state.selectedPart = "blaster"; state.previewRotation = 0; designer.renderBuildGrid(); assert.equal(elements.get("rotationIndicator").hidden, false, "Build rotation indicator visible for rotatable");
  ...
  designer.setBlueprintView("heat"); assert.equal(elements.get("buildInteractionGuide").hidden, false, "Heat guide visible"); assert.match(elements.get("buildInteractionGuide").textContent, /Hover to inspect Heat/i); assert.equal(elements.get("rotationIndicator").hidden, false, "Heat rotation indicator visible"); assert.equal(elements.get("emptyGridInstruction").hidden, false, "Heat empty instruction visible");
  designer.setBlueprintView("wiring"); assert.equal(elements.get("buildInteractionGuide").hidden, true, "Wiring guide hidden"); assert.equal(elements.get("rotationIndicator").hidden, true, "Wiring rotation hidden"); assert.equal(elements.get("emptyGridInstruction").hidden, true, "Wiring empty instruction hidden");
