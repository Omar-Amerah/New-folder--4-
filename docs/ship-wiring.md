...
mirrored by `defaultDesign()`/`defaultWiring()` on the client) includes deterministic physical Wiring v2 Power sections generated by the shared `createGeneratedPowerWiring()` helper. The topology is normalized, uses only occupied component footprint cells, joins Core, Reactor, and Aux Generator to every powered component, and is the authoritative default for client and server stock ships. Its Data wiring is empty because it has no Data-support module.
