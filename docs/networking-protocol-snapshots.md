...
Rooms now carry a monotonic `stateEpoch` and per-epoch `snapshotSeq`. Every state packet is explicit: `snapshotKind` is `full` or `compact`, compact packets declare `baseSnapshotSeq`, and snapshots include `staticRevision`, `staticRevisions`, `simulationTimeMs`, `serverTimeMs`, and `createdAtMs`. A full snapshot is the only baseline-establishing packet and contains the authorized room, phase, rules, world/map/safe zones, players, ships and designs, component HP/heat arrays, weapon angles, objectives, bullets, effects, winner/control-victory state, and protocol identifiers. Compact snapshots may omit static room fields and may be rejected atomically. `snapshotSeq` is a room-global, monotonically increasing generation identifier within the current epoch; it records when a snapshot variant was generated and may legitimately skip for any individual client. `baseSnapshotSeq` is the compact packet's continuity proof: it names the exact accepted client baseline used to build the delta. Clients accept compact snapshots only when the epoch and static revision match, `snapshotSeq` is newer than the client's accepted sequence, a full baseline exists, and `baseSnapshotSeq` equals the accepted sequence. A compact whose base is newer than the client's state is a sequence gap; a compact whose base is older or otherwise different is a wrong/stale base.
...
Each connection tracks its own baseline: epoch, `lastWrittenSeq` (the latest snapshot sequence actually handed to the socket), `lastQueuedSeq` (the sequence currently waiting in the bounded application-level snapshot queue), last full sequence, full-required flag, known static revision, queued snapshot kind, and outbound backpressure counters. Full snapshots are preserved, compact snapshots may coalesce, and slow clients are bounded independently from healthy clients. Encoded payload reuse is keyed by privacy class, epoch, kind, baseline, and static revision.
