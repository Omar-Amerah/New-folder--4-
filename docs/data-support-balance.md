...
Section 6E validation is focused on deterministic runtime, lifecycle, designer-parity and reporting coverage; final completion remains gated on this follow-up PR and complete CI passing. The reference ships in `test-fixtures/dataSupportReferenceShips.js` exercise real Wiring v2 Data sections, generated physical Power wiring, shared allocation, server runtime Data allocation, production Power brownout, production Heat state transitions, component detonation/repair lifecycle, combat/projectile execution, stable component indexes, designer predictions and measured report consumers. The balance report is generated with `npm run balance:data-support`.
...
- Component destruction and repair coverage uses production detonation and repair lifecycle entry points to rebuild runtime wiring/topology without mutating the fixture blueprint; repaired components rejoin allocation once and duplicate routes do not duplicate budgets.
- Isolated physical Data networks prove zero leakage between unrelated sources and weapons.
- Browser validation exercises the deliberately mixed critical/redundant fixture and all five reference fixtures through the browser-side designer analysis path.
...
- **Reference B — Broadside build:** one Fire Control budget is split across four Blasters. Runtime reload/cooldown derives from each Blaster's own effective fire rate, production Power brownout reduces source output, and disconnected support returns all four weapons to base fire rate.
