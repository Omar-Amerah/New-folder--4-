...
| design -> active | `ready` from every required present slot | Players/server | All required present slots ready; readiness does not validate ships or funds | Start the match with no automatic ships, set match times | Notice; static snapshot | Players buy ships during active play |
| active -> ended | Full-control victory | Server | One side holds every relay continuously for 20 seconds | Freeze winner and preserve final round statistics | Snapshot/banner | None |
...
## Attachment and rematch verification
